...
class flux_node(vapor_diffusion_base_class):
    ...
    def kE(self, node):
        ...
        # ke = dv * rh * node.S * 1000 * lambdav * eta_th
...
class vapor_flux(flux_node):
    ...
    def rdz(self):
        ...
        if self._rdz is None:
            self._rdz = self.left_node.center.distance(self.right_node.center)
        return self._rdz
    ...
    def qvT(self):
        ...
        """
        q = (self.left_node.T - self.right_node.T) *\
            (self.kE(self.left_node) + self.kE(self.right_node)) \
