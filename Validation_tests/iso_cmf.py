...
def _layers(c_iso, C_cmf, testcase):
    ...
    for lr in C_cmf.layers:
        new_layer = iso_storages.iso_soil_layer(ID=id,
                                                ...
                                                theta_0=lr.soil.theta_r,
                                                theta_sat=lr.porosity,
                                                tortuosity=lr.soil.l,
                                                T0=303.17,
                                                T=303.17,
...
def update_boundaries(c_iso, c_cmf, time):
    ...
    f = 1 / cmf.day.AsSeconds()  # factor [mm / day] to [m3 / s]
...
def iso_setup(testcase):
    ...
    # bulids storage indexing after all the cells and storages are defined
    p.build_mapping()

    return p, P


def run(P, p, sim_period=50, dt=1, solutes=["2H", "18O"], testcase=1, check_mass=True):
    ...
    c_iso_delta = {'2H': [], '18O': []}
    m_pot, theta, ql, qv = [], [], [], []
    del_eff_sat, storage_i, dS, qi_ev, q_ev, T = [], [], [], [], [], []
    ...
    for t in solver.run(start, end, timestep):
        ...
        for solute in solutes:
            dc, err = p.run(Isotopologue=solute,
                            delta_time=delta_t,
                            error_tolerance=1e-8,
                            check_mass_balance=check_mass,
                            **kwargs)
    ...
    return c_iso_delta, [theta, m_pot, ql, qv, T], \
        [error, ev, ql_surface, qv_surface]


def run_testcases(test_cases=[1], solutes=["2H", "18O"], dt=12, sim_period=250, check_mass=True):
    """"
