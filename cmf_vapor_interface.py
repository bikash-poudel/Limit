...
class Vaporizer:
    ...
    def _set_flux(self, vaporfluxes: NDArray[np.float64]):
    ...
    def _get_flux(self):
        return np.array([-nb.flux[cmf.never] / layer.get_capacity()
                         for nb, layer in zip(self.boundaries, self.cell.layers)])

    flux = property(_get_flux, _set_flux)
