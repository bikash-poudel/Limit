...
class flux_node(IsotopeRegistry):
    ...
    def get_conc_iso_liquid(self, Isotopologue):
        ...
        return self._conc_iso_liquid[0 if Isotopologue == "2H" else 1]
    ...
    def alpha_i(self, Isotopologue, T=None, **kwargs):
        ...
        try:
            ...
            if not ignore_alpha_i:
                return self._alpha_i(T, Isotopologue)
            else:
                return 1.0
    ...
    def alpha_i_horita(T: float, Isotopologue, **kwargs):

        """
        Equilibrium liquid-vapor fractionation factor α (liquid/vapor).

        Horita & Wesolowski (1994) equations.

        Parameters
        ----------
        T     : Temperature in °K
        Isotopologue : '18O' or '2H'

        Returns
        -------
        α  (dimensionless, > 1 means liquid is enriched relative to vapor)
        """
        ...
        elif Isotopologue == "2H":
            # 1000 ln α* = 1158.8(T³/10⁹) - 1620.1(T²/10⁶) + 794.84(T/10³) - 161.04 + 2.9992e9/T³
            alpha_i = (
                               1158.8 * (T ** 3 / 1e9)
                               - 1620.1 * (T ** 2 / 1e6)
                               + 794.84 * (T / 1e3)
...
class iso_atmosphere(flux_node):
    ...
    def calc_Ri_atm(self, RL, ha, T, Isotopologue):
        """
        HYDRUS - diagnostic atmospheric isotope ratio (Ra)

        Parameters
        ----------
        RL : float
            Soil liquid water isotope ratio at evaporation surface
        ha : float
            Relative humidity (0-1)
        T : atmospheric Temperature in Kelvin (K)


        Returns
        -------
        Ra : float or array
            Atmospheric vapor isotope ratio
        """

        alpha = self.alpha_i(Isotopologue=Isotopologue, T=T)

        numerator = (alpha - 1.0 + ha + ha * alpha) * RL
        denominator = 2.0 * ha
...
class iso_storage(flux_node):
    ...
    def theta(self, value):
        ...
        self._S_cache = None
        self._liq_sat_cache = None
        self._cv_cache = None  # cv depends on S via relative_humidity
    ...
    # --- psi property with auto-invalidation ---
    @property
    def psi(self):
        return self._psi
    ...
    def get_storage_i(self, Isotopologue, **kwargs):
        ...
        """
        avail_vol = kr._calc_avail_vol(
            self.theta_sat, self.thickness, self.cell.area)
        del_eff_S = self.del_eff_saturation(Isotopologue=Isotopologue, **kwargs)
        conc_iso = self._conc_iso_liquid[0 if Isotopologue == "2H" else 1]

        return kr._calc_storage_i(conc_iso, del_eff_S, avail_vol)
    ...
    def get_eff_liquid_volume(self, Isotopologue, **kwargs):
        ...
        eff_S = self.eff_saturation(Isotopologue=Isotopologue, **kwargs)

        return kr._calc_eff_liq_vol(eff_S, avail_vol)
    ...
    def get_saturation(self):
        ...
        if self._S_cache is None:
            self._S_cache = (self._theta - self.theta_0
                            ) / (self.theta_sat - self.theta_0)
    ...
    def get_liq_saturation(self):
        ...
        if self._liq_sat_cache is None:
            S = self.S
            cv = self.cv
            self._liq_sat_cache = (S - cv) / (1 - cv)

        return self._liq_sat_cache # liq_S
    ...
    def get_cv(self):
        ...
        if self._cv_cache is None:
            self._cv_cache = self.relative_humidity(psi=self.psi, T=self.T) * self.cv_sat # sli_utils.f90:L1417
    ...
    def eff_saturation(self, Isotopologue, **kwargs):
        ...
        try:
            ...
            iso_idx = 0 if Isotopologue == "2H" else 1

            alpha_i = self._alpha_i(self._T, Isotopologue)
            d_alpha = self._d_alpha(self._T, Isotopologue)

            return kr._calc_eff_S(
                self.Sl, self.cv, self.get_liq_saturation(),
                self.theta_0, self.theta_sat,
                alpha_i, d_alpha, self.dT
            )
    ...
    def del_eff_saturation(self, Isotopologue, **kwargs):
        ...
        try:
            ...
            alpha_i = self._alpha_i(self._T, Isotopologue)
            d_alpha = self._d_alpha(self._T, Isotopologue)
            has_pond = self.pond is not None
            pond_h = self.pond.pond_height_t0 if has_pond else 0.0

            return kr._calc_del_eff_S(
                self.theta_sat, self.theta_0, self.theta_t0,
                self.psi_0, self.T0,
                self.S, self._T,
                self.Sl, self.cv, self.get_liq_saturation(), self.dT,
                alpha_i, d_alpha,
                pond_h, self.thickness, has_pond
            )
    ...
    def relative_humidity(self, psi, T):
        ...
        return kr._calc_relative_humidity(psi, T, self.S)
    ...
    @staticmethod
    @lru_cache(maxsize=256)
    def _d_alpha(T, Isotopologue):
        if "2H" == Isotopologue:
            return (2 * 24844 / T ** 3 + (-76.248) / T ** 2) / exp(
                24844 / T ** 2 + (-76.248) / T + 0.052612)
        elif "18O" == Isotopologue:
            return (2 * 1137 / T ** 3 + (-0.4156) / T ** 2) / exp(
    ...
    def d_beta(self, Isotopologue, T=None, dT=None, **kwargs):
        ...
        try:
            ...
            if ignore_alpha_i:
                ...
                return self._d_alpha(T, Isotopologue) * dT
            else:
                raise ValueError
        except ValueError as err:
            print(err)
...
class iso_surface_water(iso_aquifer):
    ...
    def set_volume(self, v_new):

        self.volume = v_new

    def get_iso_mass(self, Isotopologue):
        return self.get_conc_iso_liquid(Isotopologue) * self.volume
