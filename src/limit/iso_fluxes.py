...
class flux_connection(object):
    ...
    def calc_fluxes(self, Isotopologue, **kwargs):
        """
        Returns (flux_liquid, flux_iso) in a single call.
        """
        return (
            self.calc_flux_liquid(Isotopologue=Isotopologue, **kwargs),
            self.calc_flux_i(Isotopologue=Isotopologue, **kwargs)
        )
...
class liquid_diffusion_base_class(object):
    ...
    def dl_i_self_diffusivity(T, Isotopologue, formulation="Melayah"):
        """
...
class liquid_diffusion(flux_connection, liquid_diffusion_base_class):
    ...
    def calc_flux(self, Isotopologue, dl_i_formulation="Cuntz", **kwargs):
        ...
        if self._delta_z is None:
            self._delta_z = self.left_node.center.distance(
                to_Point=self.right_node.center
            )
        ...
        dl_mean = ((dli_eff_sli_leftnode * thickness_left_node + dli_eff_sli_rightnode * thickness_right_node) / (
                thickness_right_node + thickness_left_node)) / self._delta_z

        return dl_mean * self.interface
    ...
    def calc_fluxes(self, Isotopologue, **kwargs):
        flux = self.calc_flux(Isotopologue=Isotopologue, **kwargs)
        flux_i = flux * (
                self.left_node.get_conc_iso_liquid(Isotopologue)
                - self.right_node.get_conc_iso_liquid(Isotopologue)
        )
        return flux, flux_i
...
class vapor_diffusion(flux_connection, vapor_diffusion_base_class):
    ...
    def calc_flux(self, Isotopologue, **kwargs):
        ...
        dv_i_mean = (leftnode_dvi_sli * self.left_node.thickness + rightnode_dvi_sli * self.right_node.thickness) / (
                self.left_node.thickness + self.right_node.thickness) / self._delta_z

        return dv_i_mean * self.interface
    ...
    def calc_fluxes(self, Isotopologue, **kwargs):
        ...
        # compute beta_mean once — reused for both flux_liquid and flux_iso
        T_mean = (self.left_node.T + self.right_node.T) / 2
        T_mean_0 = (self.left_node.T0 + self.right_node.T0) / 2
        dT_mean = T_mean - T_mean_0

        alpha_i_mean = self.left_node.alpha_i(
            Isotopologue=Isotopologue, T=T_mean, **kwargs)
        d_beta_mean = self.left_node.d_beta(
            Isotopologue=Isotopologue, T=T_mean, dT=dT_mean, **kwargs)
        beta_mean = alpha_i_mean + d_beta_mean

        # flux_liquid
        flux_liquid = flux * beta_mean
...
class liquid_advection(flux_connection):
    ...
    def __init__(self,
                 left_node,
                 right_node,
                 q_l=0.0, # Liquid flux in m/day positive form left to right : negative from right to left
                 climate='arid'
                 ):
        """
        Constructor of boundary_connection
    ...
    def calc_fluxes(self, Isotopologue, **kwargs):
        ...
        if self.climate == 'humid':
            flux_i = (self.left_node.get_conc_iso_liquid(Isotopologue) * flux
                      if flux >= 0.0
                      else self.right_node.get_conc_iso_liquid(Isotopologue) * flux)
        else:
            flux_i = flux * 0.5 * (
                    self.left_node.get_conc_iso_liquid(Isotopologue)
                    + self.right_node.get_conc_iso_liquid(Isotopologue)
...
class vapor_advection(flux_connection, vapor_diffusion_base_class):
    ...
    def calc_beta_mean(self, Isotopologue, **kwargs):
        ...
        return alpha_i_mean + d_beta_mean

    def calc_beta_node(self, node, Isotopologue, **kwargs):

        dT = node.T - node.T0

        alpha_i_mean = self.left_node.alpha_i(Isotopologue=Isotopologue, T=node.T, **kwargs)
        d_beta_mean = self.left_node.d_beta(Isotopologue=Isotopologue, T=node.T, dT=dT, **kwargs)
    ...
    def calc_flux_liquid(self, Isotopologue, **kwargs):
        ...
        betaqv = self.dv_i(dv=1, Isotopologue=Isotopologue, **kwargs)

        if self.climate == 'arid':

            beta_mean = self.calc_beta_mean(Isotopologue=Isotopologue, **kwargs)
            flux_v = flux * betaqv * beta_mean  # sli_solve:L2439

        elif self.climate == 'humid':

            if flux >= 0.0:

                beta = self.calc_beta_node(node=self.left_node, Isotopologue=Isotopologue, **kwargs)
            else:
                beta = self.calc_beta_node(node=self.right_node, Isotopologue=Isotopologue, **kwargs)

            flux_v = flux * betaqv * beta
        else:
            raise ValueError
    ...
    def calc_flux_i(self, Isotopologue, **kwargs):
        ...
        if self.climate == 'humid':
            ...
            if flux >= 0.0:
                ...
                beta = self.calc_beta_node(node=self.left_node, Isotopologue=Isotopologue, **kwargs)
                flux_i = self.left_node.get_conc_iso_vapor(Isotopologue) * beta * betaqv * flux

            else:

                beta = self.calc_beta_node(node=self.right_node, Isotopologue=Isotopologue, **kwargs)
                flux_i = self.right_node.get_conc_iso_vapor(Isotopologue) * beta * betaqv * flux

        elif self.climate == 'arid':

            #TODO: beta_leftnode = self.left_node.beta(Isotopologue=Isotopologue, **kwargs)   ?? ??
            #TODO: beta_rightnode = self.right_node.beta(Isotopologue=Isotopologue, **kwargs)

            beta_mean = self.calc_beta_mean(Isotopologue=Isotopologue, **kwargs)
            cqv = 0.5 * (self.left_node.get_conc_iso_liquid(Isotopologue)
                         + self.right_node.get_conc_iso_liquid(Isotopologue)) * beta_mean

            flux_i = flux * betaqv * cqv  # sli_solve:L2438
    ...
    def calc_fluxes(self, Isotopologue, **kwargs):
        ...
        if self.climate == 'arid':
            ...
            flux_liquid = flux * betaqv * beta_mean
            ...
            flux_iso = flux * betaqv * cqv

        elif self.climate == 'humid':

            if flux >= 0.0:
                beta = self.calc_beta_node(
                    node=self.left_node, Isotopologue=Isotopologue, **kwargs)
                flux_liquid = flux * betaqv * beta
                flux_iso = self.left_node.get_conc_iso_vapor(Isotopologue) \
                           * beta * betaqv * flux
            else:
                beta = self.calc_beta_node(
                    node=self.right_node, Isotopologue=Isotopologue, **kwargs)
                flux_liquid = flux * betaqv * beta
                flux_iso = self.right_node.get_conc_iso_vapor(Isotopologue) \
                           * beta * betaqv * flux
        else:
            raise ValueError
...
class evaporation(boundary_connection, vapor_diffusion_base_class, liquid_diffusion_base_class):
    ...
    def __init__(self,
        ...
        vapor_diffusion_base_class.__init__(self)
        liquid_diffusion_base_class.__init__(self, q_l, hydrodynamic_dispersivity)
        self._area = self.top_layer.cell.area
        self.q_evap = q_Evap
    ...
    def calc_flux_i(self, Isotopologue, formulation_alpha_i_k="MathieuBariac", formulation_dl_i="Cuntz", **kwargs):
        ...
        #return self.calc_flux(Isotopologue=Isotopologue) * self.top_layer.get_conc_iso_liquid(Isotopologue) # evap_flux
        return evap_flux
    ...
    def get_evap_influx(self):
        ...
        q_in = self.atmosphere.cv_a / self.rbw(ram, rbh)
    ...
    def get_evap_outflux(self):
        ...
        q_out = self.cv_surface / self.rbw(ram, rbh)
    ...
    def get_cv_surface(self):
        ...
        cv_surface = self.atmosphere.cv_a + self.q_evap * self.rbw(ram, rbh)
    ...
    def c_iso_liq_surface(self, Isotopologue, formulation_alpha_i_k="MathieuBariac", formulation_dl_i="Cuntz",
        ...
        dvi_top_layer = self.dv_i(dv=dv_top_layer, Isotopologue=Isotopologue, **kwargs) * self._area
        ...
        dl = self.dli_eff_sli(dl_eff, theta_sat=self.top_layer.theta_sat, theta_r=self.top_layer.theta_0,
                              liq_sat=self.top_layer.Sl) * self._area
        ...
        if dv_top_layer != 0.0:
            cv_toplayer = - self.qv * (0.5 * self.top_layer.thickness) / dv_top_layer / self._area + cvs
        else:
            cv_toplayer = cvs
        ...
        num = dvi_top_layer / (0.5 * self.top_layer.thickness) * cv_toplayer \
              * alphai_top_layer * self.top_layer.get_conc_iso_liquid(Isotopologue) \
              ...
              + c_iso_atm * alpha_i_k / self.rbw(ram, rbh) \
              + dl * self.top_layer.get_conc_iso_liquid(Isotopologue) / (0.5 * self.top_layer.thickness)

        den = alpha_i_k * cvs * alphai_surface / self.rbw(ram, rbh) + self.ql * (1 - w1) \
            ...
            if not ignore_alpha_i_k:
                ...
                if formulation == "Melayah":
                    ...
                    if nK_Brutsaert is not None and dv != None and dv_i is not None \
                        ...
                        alpha_i_k = 1 + nK_Brutsaert * (dv / dv_i - 1) * self.rbw(ram, rbh)  ##SLI: cable_sli_solve.f90::L2378
                    else:
    ...
    def rbw(self, ram, rbh):
        ...
        """
        rbw = (ram + rbh)   # resistance to water vapour transfer beween surface and lowest atmospheric layer
...
class ground_water(boundary_connection):
    ...
    A groundwater is the object of aquifer class
    ...
    def __init__(self,
        ...
        if isinstance(ground_water, iso_storages.iso_aquifer) \
                and isinstance(soil_layer, iso_storages.iso_soil_layer):

            boundary_connection.__init__(self, left_node=ground_water, right_node=soil_layer)
            self.soil_layer = soil_layer
    ...
    def calc_flux_i(self, Isotopologue, **kwargs):
        ...
        Positive flux of the given Isotopologue [m/day] left to right
        """
        ql = self.calc_flux(iso_storages, **kwargs)

        if ql >= 0:  # flux towards

            return self.calc_flux(Isotopologue=Isotopologue) \
                * self.soil_layer.get_conc_iso_liquid(Isotopologue=Isotopologue)

        elif ql < 0:  # flux into the soil

            return self.calc_flux(Isotopologue=Isotopologue) \
                * self.ground_water.get_conc_iso_liquid(Isotopologue=Isotopologue)


class surface_flux(boundary_connection):
    ...
    A surface water is the object of aquifer class
    ...
    def __init__(self,
                 soil_layer,
                 surface_water,
                 ql=0.0,  # positive liquid flux in (kg/(m2*day)) from node_SoilLayer to node_aquifer.
                 ):
        """
        Constructor of boundary_connection
        ...
        if isinstance(surface_water, iso_storages.iso_aquifer) \
                and isinstance(soil_layer, iso_storages.iso_soil_layer):

            boundary_connection.__init__(self, left_node=surface_water, right_node=soil_layer)
            self.soil_layer = soil_layer
    ...
    def calc_flux_i(self, Isotopologue, **kwargs):
        ...
        if ql >= 0:  # flux towards surface

            return self.calc_flux(Isotopologue=Isotopologue) \
                * self.soil_layer.get_conc_iso_liquid(Isotopologue=Isotopologue)

        elif ql < 0:  # flux into the soil from surface

            return self.calc_flux(Isotopologue=Isotopologue) \
                * self.surface_water.get_conc_iso_liquid(Isotopologue=Isotopologue)


class outlet(boundary_connection):

    """
    Description
    ===========

    outlet of the project; Neuman boundary
    """
    ...
    def __init__(self,
        ...
        """
        if isinstance(outlet, iso_storages.iso_aquifer) and isinstance(soil_layer, iso_storages.iso_soil_layer):

            boundary_connection.__init__(self, left_node=outlet, right_node=soil_layer)
            self.soil_layer = soil_layer
            self.outlet = outlet
        else:
            raise NotImplementedError
    ...
    def calc_flux_i(self, Isotopologue, **kwargs):
        ...
        if ql >= 0:  # flux towards surface
            ...
            return self.calc_flux(Isotopologue=Isotopologue) \
                * self.outlet.get_conc_iso_liquid(Isotopologue=Isotopologue)


class lateral_flux(flux_connection):
    ...
    def __init__(self, left_node, right_node, flux_type="liquid_diffusion"):

        flux_connection.__init__(self, left_node, right_node)
        self.flux_type = flux_type

        # Create the appropriate flux instance based on type
        if self.flux_type == "liquid_advection":
            self.flux_instance = liquid_advection(left_node, right_node)
        elif self.flux_type == "liquid_diffusion":
            self.flux_instance = liquid_diffusion(left_node, right_node)
        elif self.flux_type == "vapor_advection":
            self.flux_instance = vapor_advection(left_node, right_node)
        elif self.flux_type == "vapor_diffusion":
            self.flux_instance = vapor_diffusion(left_node, right_node)
        # Add other flux types as needed
        else:
            raise ValueError(f"Unknown flux type: {flux_type}")

    def __getattr__(self, name):
        # Delegate method calls to the flux_instance
        if hasattr(self.flux_instance, name):
            return getattr(self.flux_instance, name)
        raise AttributeError(f"'{self.__class__.__name__}' object has no attribute '{name}'")


class open_water_evaporation():

    """
    Evaporation flux from open water body.

    Isotope fluxes computed using the Craig-Gordon (1965) model
    with Gonfiantini (1986) kinetic fractionation.

    Internal isotope quantities carried as mass concentrations [kg m⁻³].
    Isotope ratios R [mol/mol] used only within Craig-Gordon equations.
    δ values computed only at output/interpretation stage.
    ...
    def __init__(self,
                 ...
                 q_Evap=0.0,
                 # evaporative flux from soil or litter surface to atmosphere (kg/(m2*day)) -> should be a negative number flowing from UpperSoilLayer to Atmosphere
                 T=273.0,  # soil surface (interface atmosphere / soil) temparature in Kelvin
                 ):

        self.atmosphere = atmosphere
        self.open_water = open_water_body
        self.q_evap = q_Evap

        # Liquid fluxes [m³ s⁻¹]
        self.qout = []  # non-evaporative outflow
        self.qin = []  # liquid inflow

        # ── Isotope concentration of inflow [kg m⁻³] ──
        self.conc_in = [{"2H":1.0, "18O":1.0}]

        # surface temperature [K]
        self.T = T

    def calc_delta_mass(self, delta_t, Isotopologue, **kwargs):

        """ KG """

        return self.calc_net_iso_flux(Isotopologue=Isotopologue, **kwargs) * delta_t

    def calc_net_iso_flux(self, Isotopologue, **kwargs):

        """
         Net isotope mass flux for one time step [kg s-1].

         Positive qi_net = net gain of heavy isotopologue by the storage.

         """

        qi_out = (self.calc_flux_i_evap(Isotopologue=Isotopologue, **kwargs)
                  + self.calc_flux_i_out(Isotopologue=Isotopologue))

        qi_in = self.calc_flux_i_in(Isotopologue=Isotopologue, **kwargs)

        return  qi_in - qi_out

    def calc_flux_out(self):
        """
        Non-evaporative liquid outflow from the water body [m³ s⁻¹].

        """
        return self.qout

    def calc_flux_in(self):
        """
        Non-evaporative liquid inflow to the water body [m³ s⁻¹].
        """
        return self.qin

    def calc_flux_i_out(self, Isotopologue, **kwargs):
        """
         Isotope fluxes leaving via non-evaporative outflow [kg s⁻¹].

         Outflow carries well-mixed storage concentration (correct).
        """

        return np.sum(self.qout) * self.open_water.get_conc_iso_liquid(Isotopologue)

    def calc_flux_i_in(self, Isotopologue, **kwargs):
        """
        Isotope flux entering via non-evaporative inflow [kg s⁻¹].

        """

        return np.sum([q * c[Isotopologue]
                for q, c in zip(self.qin, self.conc_in)])

    def calc_flux_i_evap(self, Isotopologue, **kwargs):

        """
        Isotope flux leaving via evaporation [kg s⁻¹].

        Craig-Gordon model in R space:
            R_E = (α*·R_L − h·R_A − ε_tot/1000) / (1 − h + ε_k/1000)

        Bug fixes
        ---------
        - R_a was fetched as concentration but used as ratio → now converted
        - Round-trip R→δ→C replaced with direct R→C conversion
        - cv_a multiplier removed (was double-counting units)

        """

        # ── Liquid storage: concentration → ratio ──────────────────
        C_l = self.open_water.get_conc_iso_liquid(Isotopologue)
        R_l = iso_delta.concentration_to_ratio(C_l, solute_i=Isotopologue)

        # ── Atmospheric vapor: concentration → ratio ────────────────
        C_a = self.atmosphere.get_conc_iso_vapor(Isotopologue)
        R_a = iso_delta.concentration_to_ratio(C_a, solute_i=Isotopologue)

        # ── Fractionation factors ───────────────────────────────────
        ha = self.atmosphere.Rh
        alpha_i = self.atmosphere.alpha_i(Isotopologue=Isotopologue,
                                          T=self.T, **kwargs)
        epsilon_k = self.epsilon_k(Isotopologue=Isotopologue)  # ‰

        # ── Craig-Gordon R_E ────────────────────────────────────────
        R_i = self.Ri_evap(RL=R_l, Ra=R_a, ha=ha,
                           alpha_i=alpha_i, ep_k=epsilon_k)

        # ── R → concentration ─────────────
        delta_i = iso_delta.ratio_to_delta(R_i=R_i, solute_i=Isotopologue)
        C_i = iso_delta.delta_to_concentration(delta_i=delta_i, solute_i=Isotopologue)

        # ── Isotope mass flux [kg s⁻¹] ──────────────────────────────
        # q_evap [m³ s⁻¹] × C_i [kg m⁻³] = [kg s⁻¹]
        return self.q_evap * C_i # * self.atmosphere.cv_a

    def Ri_evap(self, RL, Ra, ha, alpha_i, ep_k):

        """
        Net liquid water isotope flux into the water body [Not evaporation ]

        """
        eps_star = (alpha_i - 1.0) * 1000.0  # ‰  equilibrium enrichment
        eps_tot = eps_star + ep_k  # ‰  total fractionation

        return (alpha_i * RL - ha * Ra) / (1 - ha + ep_k/1000)

    def epsilon_k(self, Isotopologue):
        """
        kinetic fractination factor

        """

        if Isotopologue == "2H":
            n = 12.5
        elif Isotopologue == "18O":
            n = 14.2
        else:
            return ValueError

        h = self.atmosphere.Rh

        return n * (1 - h)

    def calc_new_volume(self, delta_t):

        v_in = np.sum(self.qin) * delta_t
        v_out = (np.sum(self.qout) + self.q_evap) * delta_t

        delta_v = v_in - v_out
        V_new = max(self.open_water.get_volume() + delta_v , 0.0)

        return V_new

    """
    def calc_new_storage_concentration(self, Isotopologue, delta_m, V_new):

        m_old = self.open_water.get_iso_mass(Isotopologue=Isotopologue)
        m_new = m_old + delta_m

        # -------update concentration ------------
        C_old = self.open_water.get_conc_iso_liquid(Isotopologue)
