...
class iso_cell(cell_node):
    ...
    def __init__(self,
        ...
        self.__connection_canopy = None
        self.__connection_surface_water_ev = None
        self.__connection_neuman = None
        self.__connection_dirichlet = None

        self.__vapor_flux_calculators = None
    ...
    def connection_to_surface_water(self):
        ...
        """
        @return: Returns the flux connection to surfacewater.

        """
        return self.__connection_to_surface_water

    @property
    def connection_surface_water_evap(self):

        """
        @return: Returns the flux connection to surfacewater evaporation.

        """
        return self.__connection_surface_water_ev

    @property
    def connection_to_canopy(self):

        return self.__connection_canopy
    ...
    def add_surface_water(self, surface_water, soil_layer, open_water_connection=False, q_surface=None):
        ...
        if open_water_connection:
            op = iso_fluxes.open_water_evaporation(atmosphere=self.atmosphere,
                                                   open_water_body=surface_water,
                                                   )
            self.__connection_surface_water_ev = op
    ...
    def add_canopy(self, canopy):
        ...
        try:
            ...
            can = iso_fluxes.open_water_evaporation(atmosphere=self.atmosphere,
                                                    open_water_body = canopy
                                                    )

            self.__connection_canopy = can
    ...
    def update_layers(self, theta, T, rH=None, psi=None):
        ...
        try:
            ...
            for lr, th, T_soil, r_H, pot in zip(self.__layers, theta, T, rH, psi):
                lr.theta_t0 = lr.theta  # update theta for previous time
    ...
    def update_aquifer(self, q_aq=0.0, c_iso={"2H": 1.0, "18O": 1.0}):
        """
        ...
        try:
            ...
            self.__connection_to_aquifer.ql = q_aq
    ...
    def update_canopy(self, c_iso={"2H": 1.0, "18O": 1.0}):
        ...
        try:
            self.__canopy.set_conc_iso_liquid(c_iso["2H"], "2H")
            self.__canopy.set_conc_iso_liquid(c_iso["18O"], "18O")
    ...
    def update_vapor_fluxes(self, vapor_fluxes=None):
        ...
        try:
            if vapor_fluxes is not None:
                ...
                if not self.__vapor_flux_calculators:
                    self.__vapor_flux_calculators = [
                        vapor_flux.vapor_flux(
                            left_node=c.left_node,
                            right_node=c.right_node,
                            top_layer=self.top_layer
                        )
                        for c in self.connections_v_adv
                    ]

                q_v = []
                for vapor, c_v_adv in zip(self.__vapor_flux_calculators,
                                         self.connections_v_adv):
                    qv = vapor.qvapor() * self.area
                    c_v_adv.q_v = qv
                    q_v.append(qv)
