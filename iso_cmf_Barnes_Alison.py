...
def cmf_boundary(P):
    ...
    # ETpot = cmf.timeseries.from_scalar(5)
...
def cmf_setup():
    ...
    L_boundaries = np.array([0.01, 0.02, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.6, 1.0,
...
def run(P, p, sim_period=50, dt=1, solutes=["2H", "18O"], testcase=1, BA=True):
    ...
    for t in solver.run(start, end, timestep):
        ...
        iso.update_storages(c_iso=c, c_cmf=C), update_boundaries(c_iso=c, c_cmf=C, time=t)
