...
def cmf_boundary(P):
    ...
    outlet.potential = - 0.8  # 1.31 (18O), 1.33 (2H) for steadystate, isothermal, -0.8 for non isothermal
...
def cmf_setup():
    ...
    #L_boundaries = np.array([0.02, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.6, 1.0,
...
def update_heat(c, dt):
    ...
    qliq = np.insert(c.liquid_fluxes, 0, c.q_evap)
    qvap = np.insert(c.vapor_fluxes, 0, 0)
...
def update_boundaries(c_iso, c_cmf, time, dt):
    ...
    c_iso.update_aquifer(ql_aq=q, c_iso={"2H": c_iso_2H, "18O": c_iso_18O})
